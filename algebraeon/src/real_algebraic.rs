...
impl<'py> PythonElementCast<'py> for PythonRealAlgebraicSet {
    fn proper_subset_cast_impl(&self, obj: &Bound<'py, PyAny>) -> Result<Self::Elem, CastError> {
        if let Ok(n) = PythonRationalSet::default().subset_cast_impl(obj) {
            return Ok(PythonRealAlgebraic {
                inner: RealAlgebraic::Rational(n.inner),
            });
        }
        Err(CastError::Type)
    }
...
impl PythonRealAlgebraic {
    ...
    pub fn isolating_interval<'py>(&self, py: Python<'py>) -> Py<PyAny> {
        let (a, b) = match self.inner.isolate() {
            RealIsolatingRegion::Rational(m) => (m, m),
            RealIsolatingRegion::Interval(a, b) => (a, b),
        };
        PyTuple::new(
            py,
            vec![
